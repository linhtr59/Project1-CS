...
int started(int month, struct Crontab crontab_data[MAX_COMMANDS], int i, int min)
{
    ...
    int day = (min / (24 * 60) + firstDay(month)) % 7;
    ...
    if (monthToInt(crontab_data[i].month) == month || strcmp(crontab_data[i].month, "*") == 0)
    {
        ...
        if (atoi(crontab_data[i].day) == date || strcmp(crontab_data[i].day, "*") == 0)
        {
            ...
            if (atoi(crontab_data[i].hour) == hour || strcmp(crontab_data[i].hour, "*") == 0)
            {
                ...
                {
                    // if crontab date is equals to the date that we are looping through
                    if (dayToInt(crontab_data[i].date) == day || strcmp(crontab_data[i].date, "*") == 0){
                        return 0;
                    }
    ...
    for (int min = 0; min < minInMonth; min++)
    {
        ...
        for (int n = 0; n < MAX_COMMANDS; n++)
        {
            ...
            else if (process[n].finishTime == 0)
            {
                ...
                process[n].finishTime--;
                printf("Command %s had finished executing. Current nrunning: %i, current invoked command: %i\n",
                process[n].name, nrunning, invokedCommand);
            }
        }
        ...
        for (int i = 0; i < cronSize; i++)
        {
            ...
            */
            ...
            { // if a command start during this minute increments nrunning and invoked commands
                ...
                printf("Command executed: %s at: %s:%s:%s:%s:%s. Current nrunning: %i, current invoked command: %i\n",
                       crontab_data[i].command, crontab_data[i].minute, crontab_data[i].hour, crontab_data[i].day, crontab_data[i].month, crontab_data[i].date, nrunning, invokedCommand);

                if (nrunning > maxCommand)
                {
                    
                    maxCommand = nrunning;
                    printf("Max Command running same time updated: %i\n",maxCommand);
                }
                ...
                for (int l = 0; l < processPointer; l++){
                    printf("Process: %s, its time left: %i\n", process[l].name, process[l].finishTime);
                }
                ...
                for (int k = 0; k < MAX_COMMANDS; k++)
                {
                    ...
                    if (strcmp(crontab_data[i].command, commands[k].name) == 0)
                    {
                        commands[k].ninvoked++;
                        found = true;
                    }
